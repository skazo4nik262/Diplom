...
- `vpn` поднялся, но `AUTH_FAILED` — неверные credentials в `.ovpn`.
...
- VLESS мёртв, всё идёт через OpenVPN — штатный режим failover, чинится заменой
  `vless-out` в `sing-box-config.json`.
- Blazor висит на JS-ошибке, контейнер жив — рестарт не лечит, смотреть консоль браузера.
